...
const AdminSchoolThemesPage: React.FC = () => {
    ...
    const escolarOrdersList = themeOrders.filter(o => o.productType === 'escolar');
    ...
    const handleSelectAll = () => {
        if (selectedThemes.length === escolarThemes.length) {
            setSelectedThemes([]);
        } else {
            setSelectedThemes(escolarThemes.map(t => t.id));
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex border-b border-slate-700">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        Pedidos Recebidos
                        {escolarOrdersList.filter(o => o.status === 'pending').length > 0 && (
                            <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">
                                {escolarOrdersList.filter(o => o.status === 'pending').length}
                            </span>
                        )}
                    </div>
                    ...
                    {escolarThemes.length > 0 && (
                        <div className="flex items-center gap-2 bg-slate-800 p-3 rounded-lg border border-slate-700">
                            <input 
                                type="checkbox" 
                                id="selectAll"
                                checked={selectedThemes.length === escolarThemes.length && escolarThemes.length > 0}
            ...
            {isModalOpen && <MultiThemeModal theme={editingTheme} onSave={handleSave} onClose={closeModal} type="escolar" />}
