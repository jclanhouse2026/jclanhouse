...
const TemplateModern: React.FC<{ data: ResumeData, layout?: 'default' | 'single-column' }> = ({ data, layout = 'default' }) => {
    ...
    const SectionTitle: React.FC<{children: React.ReactNode}> = ({children}) => (
        <h2 className="font-bold border-b-2 pb-1 mb-2 uppercase tracking-wider" style={{borderColor: color, color: color}}>
            {children}
        </h2>
    );
    
    const SidebarSectionTitle: React.FC<{children: React.ReactNode}> = ({children}) => (
        <h3 className="font-bold uppercase tracking-wider text-xs pb-1 mb-2" style={{color}}>
            {children}
        </h3>
    );
    ...
    const ProfileSection = () => (
         <div className="text-center">
            {profile.photo && (
                <img src={profile.photo} alt="Profile" className="w-24 h-24 rounded-full mx-auto object-cover mb-3" />
            )}
    ...
    if (layout === 'single-column') {
         return (
            <div className={`text-gray-800 p-6 space-y-4 ${lineHeightClasses[lineHeight || 'relaxed']}`} style={{ fontFamily: 'Arial, sans-serif', fontSize: `${fontSize}px` }}>
                <ProfileSection />
    ...
    return (
        <div className={`text-gray-800 p-4 ${lineHeightClasses[lineHeight || 'relaxed']}`} style={{ fontFamily: 'Arial, sans-serif', fontSize: `${fontSize}px` }}>
