...
const TemplateClassic: React.FC<{ data: ResumeData, fontFamily?: string }> = ({ data, fontFamily }) => {
    const { profile, summary, experiences, education, courses, informatics, languages, templateColor, fontSize, lineHeight } = data;
    ...
    const SectionTitle: React.FC<{children: React.ReactNode}> = ({children}) => (
        <h2 className="text-sm font-bold uppercase tracking-widest border-b mb-2 pb-1" style={{ borderColor: color, color }}>
            {children}
        </h2>
    );

    return (
        <div className={`text-gray-900 p-6 ${lineHeightClasses[lineHeight || 'relaxed']}`} style={{ fontFamily: fontFamily || 'Georgia, serif', fontSize: `${fontSize}px` }}>
            <header className="text-center mb-6 border-b pb-4">
                <h1 className="text-3xl font-bold tracking-wider" style={{color}}>{profile.name}</h1>
