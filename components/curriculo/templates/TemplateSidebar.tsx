...
const TemplateSidebar: React.FC<{ data: ResumeData }> = ({ data }) => {
    ...
    const SectionTitle: React.FC<{children: React.ReactNode}> = ({children}) => (
        <h3 className="font-bold uppercase tracking-wider text-sm mb-2" style={{ color, fontFamily: fontTitle || 'Inter' }}>
            {children}
        </h3>
    );
    ...
    return (
        <div className={`bg-white text-gray-800 flex ${lineHeightClasses[lineHeight || 'relaxed']}`} style={{ fontFamily: fontBody || 'Inter', fontSize: `${fontSize}px` }}>
            {/* Main Content */}
            <div className="w-2/3 p-8">
                <header className="mb-6">
                    <h1 className="text-4xl font-bold" style={{ color, fontFamily: fontTitle || 'Inter' }}>{profile.name}</h1>
                ...
                {experiences.length > 0 && <section className="mb-6">
                    ...
                    <div className="space-y-4">
                        {experiences.map(exp => (
                            <div key={exp.id}>
                                <h4 className="font-bold text-lg" style={{ fontFamily: fontTitle || 'Inter' }}>{exp.role}</h4>
                                <p className="font-semibold text-gray-700">{exp.company} | {exp.period}</p>
                                <ul className={`list-disc list-inside text-gray-600 text-sm mt-1 ${alignmentClasses[alignment || 'left']}`}>
                ...
                {education.length > 0 && <section>
                    <SectionTitle>Formação Acadêmica</SectionTitle>
                     <div className="space-y-4">
                        {education.map(edu => (
                            <div key={edu.id}>
                                <h4 className="font-bold text-lg" style={{ fontFamily: fontTitle || 'Inter' }}>{edu.degree}</h4>
